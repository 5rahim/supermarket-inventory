import { cn } from '@/lib/tailwind/tailwind-utils'
import { cva, VariantProps } from 'class-variance-authority'
import React, { useRef } from 'react'


const elementStyles = cva(null, {
   variants: {
      size: {
         sm: 'w-8 h-8',
         md: 'w-12 h-12',
         lg: 'w-16 h-16',
         xl: 'w-24 h-24',
         '2xl': 'w-32 h-32',
      },
   },
   defaultVariants: {
      size: 'md',
   },
})


export interface AvatarProps extends React.ComponentPropsWithRef<'div'>, VariantProps<typeof elementStyles> {
   src?: string | null
}

export const Avatar = React.forwardRef<HTMLDivElement, AvatarProps>((props, ref) => {
   
   const {
      children,
      className,
      size,
      src,
      ...rest
   } = props
   
   let displayImage = true
   
   function handleError() {
      displayImage = false
   }
   
   const imgRef = useRef<HTMLImageElement>(null)
   
   return (
      <>
         <div
            className={cn(
               `inline-flex rounded-full justify-center align-center flex-shrink-0 bg-gray-400`,
               elementStyles({ size }),
               className,
            )}
            {...rest}
            ref={ref}
         >
            {!displayImage && <svg viewBox="0 0 128 128" className="fill-gray-500" role="img" aria-label=" avatar">
                <path
                    className="fill-gray-200"
                    d="M103,102.1388 C93.094,111.92 79.3504,118 64.1638,118 C48.8056,118 34.9294,111.768 25,101.7892 L25,95.2 C25,86.8096 31.981,80 40.6,80 L87.4,80 C96.019,80 103,86.8096 103,95.2 L103,102.1388 Z"
                ></path>
                <path
                    className="fill-gray-200"
                    d="M63.9961647,24 C51.2938136,24 41,34.2938136 41,46.9961647 C41,59.7061864 51.2938136,70 63.9961647,70 C76.6985159,70 87,59.7061864 87,46.9961647 C87,34.2938136 76.6985159,24 63.9961647,24"
                ></path>
            </svg>}
            {displayImage && <img
                ref={imgRef}
                src={src ?? ""}
                className="w-full h-full object-cover rounded-full"
                onError={e => e.currentTarget.style.display = 'none'}
                onLoad={e => e.currentTarget.style.display = 'block'}
            />}
         </div>
      </>
   )
   
})

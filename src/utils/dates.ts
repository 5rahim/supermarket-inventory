...
export const Dates = {
   ...
   asUTC: (date: Nullish<string>) => {
      return new Date(date as string)
   },
   shortFormat: (date: Nullish<string>, lng: string) => {
      return date ? format(new Date(date), "dd/MM/yyyy", { locale: getLocale(lng) }) : ''
   },
   longFormat: (date: Nullish<string>, lng: string) => {
      return date ? format(new Date(date), "dd/MM/yy, HH:mm", { locale: getLocale(lng) }) : ''
   },
   formatDistanceToNow: (utcDate: Nullish<Date | string>, lng: string,
   ...
   mergeDateAndTime(date: Nullish<Date>, time: Nullish<number>) {
      if (date && time) {
         return new Date(addMinutes(date, time).toUTCString())
      }
      return date
   },
   dataHasPassed(date: Nullish<Date | string>) {
